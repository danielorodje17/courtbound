...
async def email_register(body: dict):
    ...
    try:
        auth_resp = await run_in_threadpool(
            lambda: supa.auth.admin.create_user({
                "email": email,
                "password": password,
                "email_confirm": True,
            })
    ...
    # First upsert without trial fields, then try to set trial
    upsert_result = await run_in_threadpool(
        lambda: supa.table("users").upsert({
    ...
    # Start trial for new user (requires migration v3)
    try:
        await run_in_threadpool(
            lambda: supa.table("users").update({
                "subscription_tier": "trial",
        ...
        logger.warning(f"Trial setup skipped for email user: {e}")
...
@router.post("/login")
async def email_login(body: dict):
    ...
    try:
        anon_client = __import__("supabase").create_client(
            os.environ["SUPABASE_URL"],
            os.environ["SUPABASE_ANON_KEY"],
        )
        auth_resp = await run_in_threadpool(
            lambda: anon_client.auth.sign_in_with_password({"email": email, "password": password})
        )
        if not auth_resp or not auth_resp.user:
            raise Exception("no user")
    except Exception as e:
        err_str = str(e).lower()
        if "invalid" in err_str or "credentials" in err_str or "password" in err_str or "email" in err_str:
    ...
    try:
        lookup = await run_in_threadpool(
            ...
            .select("id,name,picture,subscription_tier,trial_end_date")
            .eq("email", email)
            .execute()
        )
    except Exception:
        lookup = await run_in_threadpool(
            lambda: supa.table("users")
            .select("id,name,picture,subscription_tier")
            .eq("email", email)
            .execute()
        )
    if not lookup.data:
    ...
    # Check and downgrade expired trial
    try:
        if user_row.get("subscription_tier") == "trial":
            trial_end_raw = user_row.get("trial_end_date")
            if trial_end_raw:
                ted = datetime.fromisoformat(str(trial_end_raw).replace("Z", "+00:00"))
                if getattr(ted, "tzinfo", None) is None:
    ...
    await run_in_threadpool(
        lambda: supa.table("user_sessions").delete().eq("user_id", internal_user_id).lt("expires_at", now_iso).execute()
    )
    await run_in_threadpool(
        lambda: supa.table("user_sessions").insert({
            "session_token": session_token,

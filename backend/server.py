...
async def log_email(data: EmailLogCreate, current_user: UserModel = Depends(get_current_user)):
    doc = {"user_id": current_user.user_id, "college_id": data.college_id, "direction": data.direction, "subject": data.subject, "body": data.body, "coach_name": data.coach_name, "coach_email": data.coach_email, "message_type": data.message_type, "created_at": datetime.now(timezone.utc).isoformat()}
...
async def _compute_progress(uid: str, ws, we):
    """Compute actual progress for a given week window."""
    ws_str = ws.isoformat()
    we_str = we.isoformat()
    ...
    # Count call notes added in this window (embedded in tracked_colleges)
...
async def get_current_goals(current_user: UserModel = Depends(get_current_user)):
    ...
    goals_doc = await db.weekly_goals.find_one(
        {"user_id": uid, "week_start": ws.isoformat()}, {"_id": 0}
    )
...
async def get_goals_history(current_user: UserModel = Depends(get_current_user)):
    ...
    # Build 8 week windows (current + 7 past)
    ...
    # Bulk fetch tracked colleges (for new_tracks + call_notes)
    tracked_all = await db.tracked_colleges.find(
        {"user_id": uid},
        {"_id": 0, "created_at": 1, "call_notes": 1}
    ).to_list(500)

    # Bulk fetch goals
    week_starts = [ws.isoformat() for ws, _ in weeks]
    goals_docs = await db.weekly_goals.find(
        {"user_id": uid, "week_start": {"$in": week_starts}},
        {"_id": 0}
    ).to_list(8)
    ...
    for ws, we in weeks:
        ...
        calls = sum(
            1 for t in tracked_all
            for cn in (t.get("call_notes") or [])
            if cn.get("created_at") and ws_str <= cn["created_at"][:10] < we_str
        )
        goals = goals_map.get(ws_str, {})
        # Calculate achievement % (goals that were set and met)
        ...
        if set_goals:
            ...
            achievement_pct = None  # No goals set

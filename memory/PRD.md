...
### v1.38 - Phase 2: Data Migration MongoDB → Supabase (Feb 2026)
- [x] Installed supabase-py client in backend
- [x] Generated and executed `migrate_to_supabase.py` — migrated 977 records across 15 tables with zero errors
- [x] All MongoDB IDs converted to deterministic UUIDs (uuid5) — script is fully idempotent, safe to re-run
- [x] FK order respected: colleges → coaches → users → all dependent tables
- [x] Integrity verified: Duke coaches, player profile (Daniel Orodje), app_settings all confirmed in Supabase
- [x] 3 orphan records noted (1 tracked_college, 2 emails) — test data with dangling user/college refs, safely skipped
- Migration script saved at: /app/backend/migrate_to_supabase.py

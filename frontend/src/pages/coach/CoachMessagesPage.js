...
import { Send, CheckCircle, Clock, ChevronRight, MessageSquare, CalendarClock } from "lucide-react";
          ...
          <>
            <div className="space-y-3">
              {messages.map(m => {
                ...
                return (
                  <div key={m.id} className={`bg-slate-900 border rounded-xl p-4 hover:border-slate-700 transition-all ${isScheduled ? "border-blue-800/60" : "border-slate-800"}`} data-testid={`sent-msg-${m.id}`}>
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        {/* Player reply preview (only for sent messages) */}
                        {!isScheduled && m.player_reply && (

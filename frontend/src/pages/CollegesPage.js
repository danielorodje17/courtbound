...
import { Search, MapPin, Users, Plus, Check, Flag, BarChart2, ArrowLeft, ShieldCheck, Mail, MailX } from "lucide-react";
...
export default function CollegesPage() {
  ...
  const [contactFilter, setContactFilter] = useState("all"); // "all" | "contacted" | "not_contacted"
  const [repliedInstitutions, setRepliedInstitutions] = useState(new Set());
  const [notRepliedInstitutions, setNotRepliedInstitutions] = useState(new Set());

  useEffect(() => {
    fetchAllColleges();
    fetchTracked();
    fetchContactedInstitutions();
  }, []);

  useEffect(() => {
    applyFilters();
  }, [search, ncaaDivision, foreignOnly, verifiedOnly, state, allColleges, trackedOnly, trackedData, tracked, basketballDivision, contactFilter, repliedInstitutions, notRepliedInstitutions]); // eslint-disable-line
  ...
  const applyFilters = () => {
    ...
    if (contactFilter === "contacted") {
      filtered = filtered.filter(c => matchesInstitutionSet(c, repliedInstitutions));
    } else if (contactFilter === "not_contacted") {
      filtered = filtered.filter(c => matchesInstitutionSet(c, notRepliedInstitutions));
    }
  ...
  const fetchContactedInstitutions = async () => {
    try {
      const { data } = await apiRequest("get", "/player/contacted-institutions");
      const toSet = (arr) => new Set((arr || []).map(n => n.toLowerCase().trim()));
      setRepliedInstitutions(toSet(data.replied));
      setNotRepliedInstitutions(toSet(data.not_replied));
    } catch {}
  };

  // Fuzzy-match college name against a Set of institution name strings
  const matchesInstitutionSet = (college, nameSet) => {
    const cName = (college.name || "").toLowerCase().trim();
    for (const inst of nameSet) {
      if (inst === cName || cName.includes(inst) || inst.includes(cName)) return true;
    }
    return false;
  };
    ...
    <div style={{ fontFamily: "Manrope, sans-serif" }}>
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-4 mb-6 space-y-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs font-bold text-slate-500 uppercase tracking-wider mr-1">Contact Status:</span>
          {[
            { key: "all",           label: "All Colleges",      icon: null },
            { key: "contacted",     label: `Contacted${repliedInstitutions.size > 0 ? ` (${repliedInstitutions.size})` : ""}`,     icon: Mail },
            { key: "not_contacted", label: `Not Contacted${notRepliedInstitutions.size > 0 ? ` (${notRepliedInstitutions.size})` : ""}`, icon: MailX },
          ].map(({ key, label, icon: Icon }) => (
            <button
              key={key}
              data-testid={`contact-filter-${key}`}
          ...
          {contactFilter !== "all" && (
            <button
              ...
              data-testid="contact-filter-clear"
            >
              Clear
            </button>
          )}
        ...
        {/* Hint text for contact filters */}
        {contactFilter === "contacted" && (
          <p className="text-xs text-green-700 bg-green-50 border border-green-200 rounded-lg px-3 py-2">
            Showing colleges you have already replied to — you have contacted {repliedInstitutions.size} college{repliedInstitutions.size !== 1 ? "s" : ""}.
          </p>
        )}
        {contactFilter === "not_contacted" && (
          <p className="text-xs text-orange-700 bg-orange-50 border border-orange-200 rounded-lg px-3 py-2">
            Showing colleges whose coaches have messaged you but you haven't replied to yet — {notRepliedInstitutions.size} college{notRepliedInstitutions.size !== 1 ? "s" : ""} awaiting your reply.
          </p>
        )}
            ...
            >
              <div className="h-32 relative overflow-hidden">
                ...
                <div className="absolute top-3 right-3 flex gap-1.5 flex-wrap justify-end">
                  ...
                  {matchesInstitutionSet(college, repliedInstitutions) && (
                    <span className="px-2 py-0.5 rounded-full text-xs font-bold uppercase tracking-wide bg-green-600 text-white flex items-center gap-1" data-testid={`contacted-badge-${college.id}`}>
                      <Mail className="w-2.5 h-2.5" /> Contacted
                    </span>
                  )}
                  {matchesInstitutionSet(college, notRepliedInstitutions) && (
                    <span className="px-2 py-0.5 rounded-full text-xs font-bold uppercase tracking-wide bg-orange-500 text-white flex items-center gap-1" data-testid={`not-contacted-badge-${college.id}`}>
                      <MailX className="w-2.5 h-2.5" /> Reply Pending
                    </span>
                  )}
                </div>
                {/* Compare toggle */}
